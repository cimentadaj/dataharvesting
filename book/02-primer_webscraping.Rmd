...
Webscraping is the subtle art of being a ninja. You need not be an expert on many things but should have a swiss army knife of skills when it comes to data cleaning. String manipulations, subetting, clever tricks and approximations rather than exact solutions will be your companion on your day to day. This book will skip the usual start from the basics sections to directly giving your results from your efforts. The aim of this primer is to create a plot like this one:

```{r, echo = FALSE, out.width = "100%"}
knitr::include_graphics(file.path(main_dir, "elections_plot.png"))
```

This plot shows the election results for all political parties in Spain since 1978. We do not have data on this on your local computer so we'll need to find this online and scrape it. By scrape it I specifically mean to write down a little R script that will go to a website for you and manually select the data points that you tell it to. Wikipedia has such data but remember that websites change. To make this book as persistent over time as possible, we have the `history_elections_spain_ex()` function in `scrapex` which already has the website saved locally for you (the original website is https://en.wikipedia.org/wiki/Elections_in_Spain). Let's load all packages and take a look at the website.
...
On the bottom right you can see the plot that we'd like to generate. This plots has all political parties since 1978 up until the 2020 elections. The first step in webscraping is to 'read' the website into R. We can do that with `read_html` function. We pass the website link (the typical https://en.wikipedia.org string) but since we already have the website locally, we just pass it the path to the local website:
...
This is precisely the data we need. This contains the election results for all parties since 1978. The `rvest` package (which we have already loaded and also has the `read_html` function) has a very handy function called `html_table()` which automatically extracts all tables from a website into R. However, `html_table()` needs to know which website we're working with so we ned to pass it the `html_website` we read in the previous step:
...
On the first column we have the year and on each column we have the name of the political party. However, the table has some problems that we need to fix. Let's outline everything:
...
* For each political party column there are values which are not numbers (these are usually values representing foot notes such as `[k]` and others such as `Dissolved` for parties which dissolved over the years). This force some columns to be `character` columns which in fact we want all columns to be of class `numeric` to visualize these in a plot.
...
We can see all the different string values in those columns. What we'd want is to replace all non-numeric values for `NA`'s. That way when we convert these columns to numbers we won't loose any information. How can we remove these values? I went through each of these columns are wrote down the `character` values that we need to remove:
...
Now we just need to apply some basic regular expression (regex from now on) skills to remove them. Let's explain what we want to do. In the regex world the `|` stands for OR. This means that if we want to find all the words `Banned` and `Boycotted` and replace them with `NA'` we could write `Banned|Boycotted`. This literally means `Banned` OR `Boycotted`. We can take the previous `wrong_labels` vector and insert a `|` between the wrong labels:
...
Alright, don't get stressed, we'll explain this line by line. The second line references our data (`elections_data`, the one we've been working with until now). The third line uses the `mutate_if` function which works for applying a function to columns that we subset based on a criteria. Let's break down that explanation even further. You can actually read the code from above like this:
...
There we go, we don't have strings in any of the se columns anymore. Let's transform all columns into numeric and remove that first row that is empty:
