...
We can see that this API has two endpoints. The first one allows you to access data on the number of COVID cases for the American states California, Utah and New York State. The second one returns vaccination rates for the same three cities. As opposed to our example in # TODO intro API, this API does not need any authentication so we can directly go to the first endpoint to read how it works:
...
#### TODO: You can't see the results of the request objetct in this chapter.
...
TODO request printout not happening?
